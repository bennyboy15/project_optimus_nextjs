...
import { deleteSalesman } from "@/app/admin/actions/admin_actions";
...
export default function ViewSalesmen({ salesmen }: { salesmen: Salesman[] }) {
  const router = useRouter();
  const handleEdit = (salesmanID: number) => {
    router.push(`/admin/salesman/${salesmanID}`);
  };
        ...
        {salesmen.map((salesman) => (
          ...
          >
            ...
            <div className="inline-flex gap-0.5">
              ...
              <button onClick={() => handleEdit(salesman.id)} className="-ml-px border border-gray-200 hover:border-orange-500 px-3 py-2 hover:border-2 text-gray-700 transition-colors hover:bg-gray-50 hover:text-gray-900 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800 dark:hover:text-white">
                {/* Edit */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
              ...
              <form
                action={async () => {
                  await deleteSalesman(salesman.id);
                }}
              >
                <button
                  type="submit"
